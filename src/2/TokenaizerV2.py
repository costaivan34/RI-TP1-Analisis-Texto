...
def get_abreviaturas(string):
	l= re.findall('([A-Za-z]+\.)(\s[a-z]|(\,|\;|\.|\:)\s[a-z])',string) 
	return	l

def get_correo_url(string):
	l= re.findall('[\w+[.-]+@[A-Za-z]+(\.[A-Za-z]+)+',string) 
	return	l


def get_telefonos(string):
	l= re.findall('[0-9]+([.|,][0-9]+([.|,][0-9]+)?)?',string) 
	return	l

def get_nombres(string):
	l= re.findall('[A-Z]([a-z])+(\s[A-Z]([a-z])+)+',string) 
	return	l

#tokenizador para abreviaturas
def tokenizar(line):
	token=["»","<",">",".","·",",",":",";","!","¡","¿","?","#","$","%","&","/","(",")","[","]","{","}","+","-","*","_","|","°","=","'",'"']
	lista = []
	#print("linea del archivo:"+line)
	l = get_abreviaturas(line)
	if not(len(l)== 0):
		for t in l:
			globals()['lista_abr'].append(t)
	l1 = get_abreviaturas(line)
	if not(len(l)== 0):
		for t in l1:
			globals()['lista_correo'].append(t)
	l2 = get_abreviaturas(line)
	if not(len(l)== 0):
		for t in l2:
			globals()['lista_tel'].append(t)
	l3 = get_abreviaturas(line)
	if not(len(l)== 0):	
		for t in l3:
			globals()['lista_nombres'].append(t)
	line = line.lower()
	for t in token:
		line=line.replace(t,' ')	
	line = re.sub(
